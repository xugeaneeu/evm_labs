...
void matrix_scalar_mult(float* matrix, float scalar) {
  cblas_sscal(N*N, scalar, matrix, 1);
}
...
void mult_matrices(float* res, float* mat1, float* mat2) {
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, N, N, N, 1.0, mat1, N, mat2, N, 0.0, res, N);
}

void sum_matrices(float* res, float* mat1, float* mat2) {
  cblas_scopy(N*N, mat1, 1, res, 1);
  cblas_saxpy(N*N, 1.0, mat2, 1, res, 1);
}
